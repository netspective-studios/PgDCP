  ...
  return mod.SQL(ctx, state)`
    ...
    ["%1$s", ...schemas.publicSchema.ltreeExtn.searchPath].join(", ")
            ...
            CREATE TABLE %1$s.%2$s_secret(
                ...
                name text NOT NULL,
                description text,
                sym_link_%2$s_secret_id integer REFERENCES %2$s(id),
                active boolean NOT NULL DEFAULT TRUE,
                created_at timestamp with time zone NOT NULL default current_date,
                updated_at timestamp with time zone,
                deleted_at timestamp with time zone,
                ...
                REFERENCES %2$s_secret(id)
            );
            CREATE INDEX %2$s_secret_context_idx ON %1$s.%2$s_secret USING gist (context);
            CREATE INDEX %2$s_secret_name_idx ON %1$s.%2$s_secret USING btree (path);
            CREATE INDEX %2$s_secret_nature_idx ON %1$s.%2$s_secret USING gist (nature);
            CREATE INDEX %2$s_secret_path_idx ON %1$s.%2$s_secret USING gist (path);
            ...
            CREATE TABLE %2$s_secret_value_text(
                id integer GENERATED BY DEFAULT AS IDENTITY,
                %2$s_secret_id integer NOT NULL REFERENCES %1$s.%2$s(id),
                value text not null
            );

            CREATE OR REPLACE VIEW %1$s.%2$s_secret_text AS
                select %2$s_secret.id as %2$s_secret_id,
                    nature, value, context, path, name, description,
                    %2$s_secret_value_text.id as value_id
                from %1$s.%2$s_secret, %1$s.%2$s_secret_value_text
                where %2$s_secret.id = %2$s_secret_value_text.%2$s_secret_id
                and active = true;

            create or replace function %1$s.variant_insert_%2$s_secret_text() returns trigger as $genBody$
            declare
                %2$sId integer;
            begin
                insert into %1$s.%2$s_secret (nature, context, path, name, description) select 
                    (CASE WHEN (NEW.nature IS NULL) THEN 'text' ELSE NEW.nature END),
                    (CASE WHEN (NEW.context IS NULL) THEN '%3$s' ELSE NEW.context END),
                    (CASE WHEN (NEW.path IS NULL) THEN '%4$s' ELSE NEW.path END),
                    NEW.name,
                    NEW.description 
                    returning id into %2$sId;
                insert into %1$s.%2$s_secret_value_text (%2$s_secret_id, value) select %2$sId, NEW.value;
                return NEW;
            end;
            $genBody$ language plpgsql;

            create trigger variant_insert_%2$s_secret_text_trigger
            instead of insert on %1$s.%2$s_secret_text
            for each row execute function %1$s.variant_insert_%2$s_secret_text();
