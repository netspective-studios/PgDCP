...
* `[AGorS]_doctor_*`. TODO: Runtime dependency checks that applications and services can run to see if all required dependencies are installed (e.g. extensions).
...
* Add *doctor* functionality, generated by `SQLa` Controller, to verify that all dependencies (e.g. extensions required) are installed before attempting to run SQL.
