...
CREATE OR REPLACE PROCEDURE ${ctx.fn.deploy.destroy('http_client_graphql')}() AS $$
BEGIN
    DROP FUNCTION IF EXISTS ${ctx.schema.assurance}.test_content_assembler_text_manipulation();
...
CREATE OR REPLACE FUNCTION ${ctx.schema.assurance}.test_http_client_graphql() RETURNS SETOF TEXT LANGUAGE plpgsql AS $$
BEGIN 
