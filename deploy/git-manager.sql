...
CREATE OR REPLACE PROCEDURE ${ctx.fn.deploy.destroy('git_management')}() AS $$
BEGIN
    DROP FUNCTION IF EXISTS ${ctx.schema.assurance}.test_git_management();
...
CREATE OR REPLACE FUNCTION ${ctx.schema.assurance}.test_git_management() RETURNS SETOF TEXT LANGUAGE plpgsql AS $$
BEGIN 
