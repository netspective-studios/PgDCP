...
CREATE OR REPLACE PROCEDURE ${ctx.fn.deploy.construct('image_management')}() AS $$
BEGIN
...
CREATE OR REPLACE FUNCTION inspect_image_meta_data(provenance text, image bytea) RETURNS image_meta_data AS $$
...
CREATE OR REPLACE PROCEDURE ${ctx.fn.deploy.destroy('image_management')}() AS $$
BEGIN
    DROP FUNCTION IF EXISTS ${ctx.schema.assurance}.test_image_management();
    DROP FUNCTION IF EXISTS image_format_size(bytea);
    DROP TYPE IF EXISTS image_format_size_type;
END;
$$ LANGUAGE PLPGSQL;

CREATE OR REPLACE FUNCTION ${ctx.schema.assurance}.test_image_management() RETURNS SETOF TEXT LANGUAGE plpgsql AS $$
DECLARE
